...
def worker(
    ...
    for fi in (
        list(
            range(
                base_index + i * worker_nodes * gpu_batchsize,
                min(
                    base_index + i * worker_nodes * gpu_batchsize + gpu_batchsize,
                    total_frames,
                ),
            )
        )
        ...
        if script_net is None:
            script_net = torch.jit.trace(
                net,
                torch.as_tensor(
                    np.stack(input_frames), dtype=torch.float32, device=DEVICE
                ),
            )
...
def matte_key(
    ...
    if framerate == -1:
        print(
            f"FRAME RATE DETECTED: {fr} (if this looks wrong, override the frame rate)"
        )
...
def transparentgif(
    ...
    cmd = (
        "nice -10 ffmpeg -y -i %s -i %s -filter_complex '[1][0]scale2ref[mask][main];[main][mask]alphamerge=shortest=1,fps=10,split[s0][s1];[s0]palettegen[p];[s1][p]paletteuse' -shortest %s"
...
def transparentgifwithbackground(
    output,
    overlay,
    file_path,
    worker_nodes,
    ...
    cmd = (
        "nice -10 ffmpeg -y -i %s -i %s -i %s -filter_complex '[1][0]scale2ref[mask][main];[main][mask]alphamerge=shortest=1[fg];[2][fg]overlay=(main_w-overlay_w)/2:(main_h-overlay_h)/2:format=auto,fps=10,split[s0][s1];[s0]palettegen[p];[s1][p]paletteuse' -shortest %s"
...
def transparentvideo(
    ...
    print("Starting alphamerge")
    cmd = (
        "nice -10 ffmpeg -y -nostats -loglevel 0 -i %s -i %s -filter_complex '[1][0]scale2ref[mask][main];[main][mask]alphamerge=shortest=1' -c:v qtrle -shortest %s"
        % (file_path, temp_file, output)
    )
...
def transparentvideoovervideo(
    ...
    print("Starting alphamerge")
    cmd = (
        "nice -10 ffmpeg -y -i %s -i %s -i %s -filter_complex '[1][0]scale2ref[mask][main];[main][mask]alphamerge=shortest=1[vid];[vid][2:v]scale2ref[fg][bg];[bg][fg]overlay=shortest=1[out]' -map [out] -shortest %s"
        % (file_path, temp_file, overlay, output)
    )
    sp.run(shlex.split(cmd))
    print("Process finished")
...
def transparentvideooverimage(
    ...
    cmd = (
        "nice -10 ffmpeg -i %s -i %s -filter_complex 'scale2ref[img][vid];[img]setsar=1;[vid]nullsink' -q:v 2 %s"
        % (overlay, file_path, temp_image)
    )
    sp.run(shlex.split(cmd))
    print("Starting alphamerge")
    cmd = (
        "nice -10 ffmpeg -y -i %s -i %s -i %s -filter_complex '[0][1]scale2ref[img][vid];[img]setsar=1[img];[vid]nullsink; [img][2]overlay=(W-w)/2:(H-h)/2' -shortest %s"
        % (
...
def download_files_from_github(path, model_name):
    if model_name == "u2net":
        ...
        try:
            ...
            sp.run(
                ["cat", part1.name, part2.name, part3.name, part4.name, ">", path],
                stdout=sp.DEVNULL,
            )
        finally:
