...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.use_rpb:
            relative_position_bias = self.rpb_table[
...
class PatchEmbed(nn.Module):
    ...
    def __init__(
        self, img_size=224, patch_size=16, in_chans=3, embed_dim=768, mask_cent=False
    ):
        super().__init__()
...
class CnnHead(nn.Module):
    ...
    def forward(self, x):
        x = rearrange(
            x, "b (p1 p2) c -> b c p1 p2", p1=self.window_size, p2=self.window_size
        )
        x = self.head(x)
    ...
    def __init__(
        ...
        for i in range(window_size):
            for j in range(window_size):
                ...
                edx, edy = min(i + kernel_size // 2, window_size - 1), min(
                    j + kernel_size // 2, window_size - 1
                )
...
class IColoriT(nn.Module):
    ...
    def forward_features(self, x, mask):
        ...
        if mask.dim() == 2:
            ...
            _full_mask = F.interpolate(
                _mask, scale_factor=hint_size
            )  # Needs to be Float
            full_mask = _full_mask.type(f"torch{_device}.BoolTensor")

            # mask ab channels
            if self.avg_hint:
                _avg_x = F.interpolate(
                    x, size=(H // hint_size, W // hint_size), mode="bilinear"
                )
                _avg_x[:, 1, :, :].masked_fill_(mask.squeeze(1), 0)
                _avg_x[:, 2, :, :].masked_fill_(mask.squeeze(1), 0)
                x_ab = F.interpolate(_avg_x, scale_factor=hint_size, mode="nearest")[
                    :, 1:, :, :
                ]
            ...
            mask.dim() == 4
        ):  # (B, 3, H, W) Zhang mode. channel 0,1 is ab. channel2 is mask
            # TODO ONLY FOR DEMO
            _device = ".cuda" if x.device.type == "cuda" else ""
            _full_mask_ab = mask.type(f"torch{_device}.FloatTensor")[:, :2, :, :]
            x[:, 1:, :, :] = _full_mask_ab
            _full_mask = (
                mask[:, 2, :, :].unsqueeze(1).type(f"torch{_device}.FloatTensor")
            )
        else:
        ...
        x = (
            x + self.pos_embed.type_as(x).to(x.device).clone().detach()
        )  # (B, 14*14, 768)
