from PySide6.QtWidgets import QWidget, QToolButton, QHBoxLayout, QScrollArea
...
class QToolInstagramFilters(QScrollArea):
    def __init__(self, parent=None, toolInput=None):
        ...
        unfilteredPixmap = unfilteredPixmap.scaled(
            buttonIconSize, QtCore.Qt.AspectRatioMode.KeepAspectRatio
        )
        icon = QIcon(unfilteredPixmap)
        noFilterButton.setIcon(icon)
        noFilterButton.setIconSize(buttonIconSize)
        noFilterButton.setToolButtonStyle(
        ...
        for i, f in enumerate(filters):
            ...
            filteredPixmap = filteredPixmap.scaled(
                buttonIconSize, QtCore.Qt.AspectRatioMode.KeepAspectRatio
            )
            ...
            filterButton.setToolButtonStyle(
                QtCore.Qt.ToolButtonStyle.ToolButtonTextUnderIcon
            )
    ...
    def OnFilterSelect(self):
        ...
        self.parent.image_viewer.show_image(
            self.parent.ImageToQPixmap(self.output), False
        )
