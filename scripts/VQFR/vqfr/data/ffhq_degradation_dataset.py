...
from scripts.VQFR.vqfr.data import degradations as degradations
from scripts.VQFR.vqfr.data.data_util import paths_from_folder
from scripts.VQFR.vqfr.data.transforms import augment
from scripts.VQFR.vqfr.utils import FileClient, get_root_logger, imfrombytes, img2tensor
from scripts.VQFR.vqfr.utils.registry import DATASET_REGISTRY
...
class FFHQDegradationDataset(data.Dataset):
    def __init__(self, opt):
        ...
        logger.info(
            f"Blur: blur_kernel_size {self.blur_kernel_size}, "
        ...
        if self.color_jitter_prob is not None:
            logger.info(
                f"Use random color jitter. Prob: {self.color_jitter_prob}, "
    ...
    def get_component_coordinates(self, index, status):
        ...
        if status[0]:  # hflip
            ...
            components_bbox["left_eye"][0] = (
                self.out_size - components_bbox["left_eye"][0]
            )
            components_bbox["right_eye"][0] = (
                self.out_size - components_bbox["right_eye"][0]
            )
