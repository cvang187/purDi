...
class VQFRv2Model(BaseModel):
    ...
    def init_training_settings(self):
        ...
        if train_opt.get("quant_feature_opt"):
            self.cri_quant_feature = build_loss(train_opt["quant_feature_opt"]).to(
                self.device
            )
        else:
        ...
        if train_opt.get("quant_index_opt"):
            self.cri_quant_index = build_loss(train_opt["quant_index_opt"]).to(
                self.device
            )
        else:
    ...
    def setup_optimizers(self):
        ...
        self.optimizer_sr_enc = self.get_optimizer(
            optim_type, encoder_params, **train_opt["optim_sr_enc"]
        )
        self.optimizers.append(self.optimizer_sr_enc)

        optim_type = train_opt["optim_sr_maindec"].pop("type")
        self.optimizer_sr_maindec = self.get_optimizer(
            optim_type, main_decoder_params, **train_opt["optim_sr_maindec"]
        )
    ...
    def optimize_parameters(self, current_iter):
        ...
        if self.cri_quant_feature:
            l_quant_feature = self.cri_quant_feature(
                lq_res["enc_feat"], hq_res["quant_feat"]
            )
            loss_dict["l_quant_feature"] = l_quant_feature.detach().mean()
            l_total_g += l_quant_feature

        if self.cri_quant_index:
            l_quant_index = self.cri_quant_index(
                lq_res["quant_logit"], hq_res["quant_index"]
            )
        ...
        if self.cri_gan and current_iter > self.opt["train"].get("gan_start_iter", 0):
            ...
            l_d = self.cri_patchgan(real_pred, True, is_disc=True) + self.cri_patchgan(
                fake_pred, False, is_disc=True
            )
    ...
    def test(self):
        self.net_sr.eval()
        with torch.no_grad():
            self.output = self.net_sr(
                self.lq, fidelity_ratio=self.opt["val"].get("fidelity_ratio", 1.0)
            )
