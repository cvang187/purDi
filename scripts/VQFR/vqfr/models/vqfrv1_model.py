...
from scripts.VQFR.vqfr.archs import build_network
from scripts.VQFR.vqfr.losses import build_loss
from scripts.VQFR.vqfr.losses.losses import r1_penalty
from scripts.VQFR.vqfr.metrics import calculate_metric
from scripts.VQFR.vqfr.utils import get_root_logger, imwrite, tensor2img
from scripts.VQFR.vqfr.utils.registry import MODEL_REGISTRY
...
class VQFRv1Model(BaseModel):
        ...
        if "network_d_global" in self.opt:
            ...
            if self.opt.get("syncbn") is True and self.opt["num_gpu"] > 1:
                self.net_d_global = torch.nn.SyncBatchNorm.convert_sync_batchnorm(
                    self.net_d_global
                )  # to avoid broadcast buffer error
    ...
    def setup_optimizers(self):
        ...
        self.optimizer_sr = self.get_optimizer(
            optim_type, optim_params, **train_opt["optim_sr"]
        )
        ...
        self.optimizer_d_local = self.get_optimizer(
            optim_type, self.net_d_local.parameters(), **train_opt["optim_d"]
        )
        self.optimizer_d_global = self.get_optimizer(
            optim_type, self.net_d_global.parameters(), **train_opt["optim_d"]
        )
        self.optimizer_d_main_local = self.get_optimizer(
            optim_type, self.net_d_main_local.parameters(), **train_opt["optim_d"]
        )
        self.optimizer_d_main_global = self.get_optimizer(
            optim_type, self.net_d_main_global.parameters(), **train_opt["optim_d"]
        )
        self.optimizers.extend(
            [
                self.optimizer_d_local,
                self.optimizer_d_global,
                self.optimizer_d_main_local,
                self.optimizer_d_main_global,
            ]
        )
    ...
    def calculate_adaptive_weight(self, nll_loss, g_loss, last_layer=None):
        if last_layer is not None:
            ...
            nll_grads = torch.autograd.grad(
                nll_loss, self.last_layer[0], retain_graph=True, allow_unused=True
            )[0]
            g_grads = torch.autograd.grad(
                g_loss, self.last_layer[0], retain_graph=True, allow_unused=True
            )[0]
    ...
    def optimize_parameters(self, current_iter):
        ...
        if self.cri_perceptual:
            ...
            l_g_main_percep, l_g_main_style = self.cri_perceptual(
                lq_res["main_dec"], self.gt
            )
        ...
        if current_iter > self.opt["train"].get("gan_start_iter", 0):
            ...
            d_weight = self.generator_d_local_weight * self.calculate_adaptive_weight(
                ...
                else self.net_sr.get_last_layer(),
            )
        ...
        if current_iter > self.opt["train"].get("gan_start_iter", 0):
            ...
            l_d = self.cri_globalgan(
                real_pred, True, is_disc=True
            ) + self.cri_globalgan(fake_pred, False, is_disc=True)
            ...
            l_d = 0.5 * (
                self.cri_patchgan(real_pred, True, is_disc=True)
                + self.cri_patchgan(fake_pred, False, is_disc=True)
            )
