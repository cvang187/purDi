...
from scripts.VQFR.vqfr.archs.stylegan_arch import UpFirDnSmooth
from scripts.VQFR.vqfr.ops.upfirdn2d import upfirdn2d
...
class ConvBlock(nn.Module):
    def __init__(
        self, in_channel, out_channel, blur_kernel=[1, 3, 3, 1], equalize=True
    ):
        super().__init__()

        self.conv1 = ConvLayer(in_channel, in_channel, 3, equalize=equalize)
        self.conv2 = ConvLayer(
            in_channel, out_channel, 3, downsample=True, equalize=equalize
        )
...
class FromRGB(nn.Module):
    def __init__(
        self, out_channel, downsample=True, blur_kernel=[1, 3, 3, 1], equalize=True
    ):
        super().__init__()
    ...
    def __init__(self, out_size, channel_multiplier=2, resample_kernel=[1, 3, 3, 1]):
        ...
        for i in range(log_size, 2, -1):
            ...
            self.from_rgbs.append(
                FromRGB(in_channel, downsample=i != log_size, equalize=False)
            )
            self.convs.append(
                ConvBlock(in_channel, out_channel, resample_kernel, equalize=False)
            )
