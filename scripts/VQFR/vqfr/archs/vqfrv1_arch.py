...
from scripts.VQFR.vqfr.archs.vqganv1_arch import (
    GeneralizedQuantizer,
    ResnetBlock,
    UpDownSample,
    VQGANDecoder,
    VQGANEncoder,
)
from scripts.VQFR.vqfr.ops.dcn import ModulatedDeformConvPack, modulated_deform_conv
from scripts.VQFR.vqfr.utils import get_root_logger
from scripts.VQFR.vqfr.utils.registry import ARCH_REGISTRY
...
class TextureWarpingModule(nn.Module):
    def __init__(
        ...
        self.downsample = UpDownSample(
            ...
            scale_factor=cond_downscale_rate,
            direction="down",
        )
...
class MainDecoder(nn.Module):
    def __init__(
        ...
        for idx, scale in enumerate(range(self.log_size + 1)):
            self.channel_dict["Level_%d" % 2**scale] = (
                channel_multipliers[idx] * base_channels
            )
            self.resolution_scalerate_dict[
                "Level_%d" % 2**scale
            ] = resolution_scale_rates[idx]
        ...
        for scale in range(self.log_size - 1, -1, -1):
            in_channel = (
                ...
                else self.channel_dict["Level_%d" % 2 ** (scale + 1)]
            )
            stage_channel = self.channel_dict["Level_%d" % 2**scale]
            upsample_rate = self.resolution_scalerate_dict["Level_%d" % 2**scale]

            self.decoder_dict["Level_%d" % 2**scale] = ResnetBlock(
                2 * stage_channel, stage_channel
            )
        ...
        if align_opt is not None:
            for level_name, level_cfg in align_opt.items():
                ...
                previous_offset_channel = (
                    0
                    if (2**level_idx) == align_from_patch
                    else self.channel_dict["Level_%d" % (2 ** (level_idx + 1))]
                )
        ...
        self.conv_out_1 = nn.Sequential(
            nn.GroupNorm(
                num_groups=32,
                num_channels=self.channel_dict["Level_%d" % 2**0],
                eps=1e-6,
                affine=True,
            ),
            nn.SiLU(inplace=True),
        ...
        self.conv_out = nn.Sequential(
            nn.Conv2d(
                self.channel_dict["Level_%d" % 2**0] + 3,
                3,
                kernel_size=3,
                stride=1,
                padding=1,
    ...
    def forward(self, dec_res_dict, prior, x_lq):
        x, offset = self.align_func_dict["Level_%d" % 2**self.log_size](
            dec_res_dict["Level_%d" % 2**self.log_size], prior
        )
