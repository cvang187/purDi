...
from scripts.VQFR.vqfr.archs.quantizer_arch import build_quantizer
...
class VQGANEncoder(nn.Module):
    def __init__(
        ...
        self.conv_in = torch.nn.Conv2d(
            3,
            self.channel_dict["Level_%d" % 2**0],
            kernel_size=3,
            stride=1,
            padding=1,
        )
        ...
        for scale in range(self.log_size + 1):
            ...
            self.encoder_dict["Level_%d" % 2**scale] = nn.Sequential(
                *[
                    ResnetBlock(stage_channel, stage_channel)
        ...
        for level_name in self.quant_level:
            ...
            self.enc_convout_dict[level_name] = nn.Sequential(
                nn.GroupNorm(
                    num_groups=32, num_channels=channel, eps=1e-6, affine=True
                ),
...
class VQGANDecoder(nn.Module):
    ...
    def forward(self, quant_res_dict, return_feat=False):
        ...
        x = quant_res_dict[
            "Level_%d" % 2**self.log_size
        ]  # set quant final as the begin of decoder
...
class GeneralizedQuantizer(nn.Module):
    ...
    def forward(self, enc_dict, iters=-1):
        ...
        for level_name in self.quantize_dict.keys():
            h_q, emb_loss, extra_info = self.quantize_dict[level_name](
                enc_dict[level_name], iters=iters
            )
