...
class FAN(nn.Module):
    def __init__(
        self, num_modules=4, end_relu=False, gray_scale=False, num_landmarks=98
    ):
    ...
    def _putGaussianMap(
        self, center, visible_flag, crop_size_y, crop_size_x, stride, sigma
    ):
        """
        根据一个中心点,生成一个heatmap
        :param center:
    ...
    def _putGaussianMaps(self, keypoints, crop_size_y, crop_size_x, stride, sigma):
        ...
        for k in range(point_num):
            ...
            heatmap = self._putGaussianMap(
                all_keypoints[k], flag, crop_size_y, crop_size_x, stride, sigma
            )
            heatmap = heatmap[np.newaxis, ...]
            heatmaps_this_img.append(heatmap)
        heatmaps_this_img = np.concatenate(
            heatmaps_this_img, axis=0
