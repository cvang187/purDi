...
from scripts.VQFR.vqfr.utils import all_gather, get_root_logger
from scripts.VQFR.vqfr.utils.registry import QUANTIZER_REGISTRY
...
class NewReservoirSampler(nn.Module):
    ...
    def _load_from_state_dict(self, state_dict, prefix, *args, **kwargs):
        ...
        return super(NewReservoirSampler, self)._load_from_state_dict(
    ...
    def add(self, samples):
        ...
        if len(samples) > 0:
            positions = torch.randint(
                0, self.i, size=(len(samples),), device=cuda_device
            )
...
class L2VectorQuantizerKmeans(nn.Module):
    ...
    def forward(self, z, iters=-1):
        ...
        return (
            z_quant,
            loss,
            {"z_conv": z_conv, "z_quant_before_conv": z_quant_before_conv},
        )
