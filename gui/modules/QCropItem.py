...
class HandleItem(QGraphicsRectItem):
    def __init__(self, position_flags, parent, imageSize):
        ...
        QGraphicsRectItem.__init__(
            self, start, start, handleItemSize, handleItemSize, parent
        )
    ...
    def restrictPosition(self, newPos):
        ...
        if (
            self.positionflags() & SizeGripItem.Top
            or self.positionflags() & SizeGripItem.Bottom
        ):
            retVal.setY(newPos.y())

        if (
            self.positionflags() & SizeGripItem.Left
            or self.positionflags() & SizeGripItem.Right
        ):
            retVal.setX(newPos.x())

        if (
            self.positionflags() & SizeGripItem.Top
            and retVal.y() > self.parentItem()._rect.bottom()
        ):
            retVal.setY(self.parentItem()._rect.bottom())

        elif (
            self.positionflags() & SizeGripItem.Bottom
            and retVal.y() < self.parentItem()._rect.top()
        ):
            retVal.setY(self.parentItem()._rect.top())

        if (
            self.positionflags() & SizeGripItem.Left
            and retVal.x() > self.parentItem()._rect.right()
        ):
            retVal.setX(self.parentItem()._rect.right())

        elif (
            self.positionflags() & SizeGripItem.Right
            and retVal.x() < self.parentItem()._rect.left()
        ):
